...
class Generator_S(torch.nn.Module):
    def __init__(self, h):
...
class Generator_T(torch.nn.Module):
    def __init__(self, h):
...
def ada_loss(fmap_s, fmap_t):
    loss = 0
    ada_losses = []
    for s, t in zip(fmap_s, fmap_t):
        l_losses = 0
        s = s.reshape(s.shape[0], -1)
        t = t.reshape(t.shape[0], -1)
        cs_s = torch.matmul(s, s.T) / torch.matmul(torch.linalg.norm(s, axis=1).reshape(-1, 1),
                                                   torch.linalg.norm(s, axis=1).reshape(1, -1))
        cs_t = torch.matmul(t, t.T) / torch.matmul(torch.linalg.norm(t, axis=1).reshape(-1, 1),
                                                   torch.linalg.norm(t, axis=1).reshape(1, -1))
        # calculate kl-dist
        for i, (cs_s_i, cs_t_i) in enumerate(zip(cs_s, cs_t)):
            cs_s_i = torch.softmax(torch.cat((cs_s_i[:i], cs_s_i[i+1:]), 0), 0)
            cs_t_i = torch.softmax(torch.cat((cs_t_i[:i], cs_t_i[i+1:]), 0), 0)
            l_losses += F.kl_div(cs_t_i.log(), cs_s_i, reduction='mean')
        l_losses /= cs_s.shape[0]
        loss += l_losses
        ada_losses.append(l_losses)

    return loss, ada_losses
